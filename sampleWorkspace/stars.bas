...
' FUNCTION() - Splash Screen / Restart Game function
PR.s

' Set graphics mode to 17 for the splash screen, also so print #6 still works with the custom display list
g.17 
...
' Pause for vertical to avoid flicker due to narrow playfield change
pause
if _lives
...
' Clear all variables when the game restarts
clr
...
dpoke i+2,dpeek(d.560+4) ' Top status rows uses existing buffer so print #6 works
...
' Each wave index is a set of possible wave parts
' Waves
